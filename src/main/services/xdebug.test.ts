...
describe('Xdebug Service', () => {
  ...
  describe('initializeXdebugStatus', () => {
    ...
    it('should fallback to file check if database fails', async () => {
      ;(database.getSetting as Mock).mockRejectedValue(new Error('Database error'))
  ...
  describe('getXdebugStatus', () => {
    it('should return true when Xdebug is enabled', async () => {
      ...
      expect(database.saveSetting).toHaveBeenCalledWith('xdebug_enabled', 'true')
    })
    ...
    it('should return false when Xdebug is disabled', async () => {
      ...
      expect(database.saveSetting).toHaveBeenCalledWith('xdebug_enabled', 'false')
    })
    ...
    it('should handle database save failure gracefully', async () => {
      const mockContent = 'xdebug.mode = develop,debug\n'
      ;(fs.readFile as Mock).mockResolvedValue(mockContent)
      ;(database.saveSetting as Mock).mockRejectedValue(new Error('Database error'))
  ...
  describe('toggleXdebug', () => {
    ...
    beforeEach(() => {
      ;(spawn as Mock).mockReturnValue(mockSpawnProcess as unknown as ReturnType<typeof spawn>)
    })
    ...
    it('should enable Xdebug when currently disabled', async () => {
      ...
      expect(spawn).toHaveBeenCalledWith('docker', ['compose', 'restart', 'php'], {
        cwd: '/test/project',
        shell: false
      })
      expect(mockMainWindow.webContents.send).toHaveBeenCalledWith('xdebug-status', {
        status: 'restarting',
        enabled: true
      })
    ...
    it('should handle file write errors', async () => {
      ...
      await expect(xdebugService.toggleXdebug(mockMainWindow)).rejects.toThrow('Write failed')
    ...
    it('should handle spawn process errors', async () => {
      ...
      await expect(promise).rejects.toThrow('Failed to spawn docker restart command')
