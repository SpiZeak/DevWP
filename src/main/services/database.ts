...
export async function saveSiteConfiguration(site: SiteConfiguration): Promise<void> {
  return new Promise((resolve, reject) => {
    ...
    const multisiteTypeEscaped = multisiteType ? escapeSqlString(multisiteType) : 'NULL'.replace(/'/g, "")
      ...
      try {
        ...
        for (let i = 1; i < lines.length; i++) {
          ...
          const site: SiteConfiguration = {
            ...
            multisite: {
              enabled: values[3] === '1',
              type: (values[4] === 'subdomain' || values[4] === 'subdirectory') ? values[4] as 'subdomain' | 'subdirectory' : 'subdomain'
            },
    ...
    exec(selectCmd, (error, stdout, stderr) => {
      ...
      try {
        ...
        const site: SiteConfiguration = {
          ...
          multisite: {
            enabled: values[3] === '1',
            type: (values[4] && values[4] !== 'NULL') ? (values[4] as 'subdomain' | 'subdirectory') : 'subdomain'
          },
          createdAt: new Date(values[5]),
          updatedAt: new Date(values[6])
        }
