...
describe('Settings IPC Handlers', () => {
  ...
  describe('registerSettingsHandlers', () => {
    it('should register all settings handlers', () => {
      ...
      expect(ipcMain.handle).toHaveBeenCalledWith('save-setting', expect.any(Function))
      expect(ipcMain.handle).toHaveBeenCalledWith('delete-setting', expect.any(Function))
      expect(ipcMain.handle).toHaveBeenCalledWith('get-webroot-path', expect.any(Function))
      expect(ipcMain.handle).toHaveBeenCalledWith(
        'get-xdebug-enabled-setting',
        expect.any(Function)
      )
      expect(ipcMain.handle).toHaveBeenCalledWith('pick-directory', expect.any(Function))
    })
  })
  ...
  describe('get-settings handler', () => {
    ...
    it('should return empty object on error', async () => {
      ...
      vi.mocked(database.getAllSettings).mockRejectedValue(new Error('Database error'))
  ...
  describe('get-setting handler', () => {
    ...
    it('should return null on error', async () => {
      const consoleError = vi.spyOn(console, 'error').mockImplementation(() => {})
      vi.mocked(database.getSetting).mockRejectedValue(new Error('Database error'))
  ...
  describe('save-setting handler', () => {
    it('should save a setting successfully', async () => {
      ...
      expect(database.saveSetting).toHaveBeenCalledWith('test-key', 'test-value')
      expect(result).toEqual({ success: true })
    })

    it('should return error on failure', async () => {
      const consoleError = vi.spyOn(console, 'error').mockImplementation(() => {})
      vi.mocked(database.saveSetting).mockRejectedValue(new Error('Save failed'))
  ...
  describe('delete-setting handler', () => {
    ...
    it('should return error on failure', async () => {
      const consoleError = vi.spyOn(console, 'error').mockImplementation(() => {})
      vi.mocked(database.deleteSetting).mockRejectedValue(new Error('Delete failed'))
  ...
  describe('get-webroot-path handler', () => {
    ...
    it('should return default path on error', async () => {
      const consoleError = vi.spyOn(console, 'error').mockImplementation(() => {})
      vi.mocked(database.getWebrootPath).mockRejectedValue(new Error('Database error'))
  ...
  describe('get-xdebug-enabled-setting handler', () => {
    it('should return Xdebug enabled status', async () => {
      ...
      const handler = vi
        .mocked(ipcMain.handle)
        .mock.calls.find((call) => call[0] === 'get-xdebug-enabled-setting')?.[1]
    ...
    it('should return false on error', async () => {
      const consoleError = vi.spyOn(console, 'error').mockImplementation(() => {})
      vi.mocked(database.getXdebugEnabledSetting).mockRejectedValue(new Error('Database error'))
