...
const BuildLog: React.FC<BuildLogProps> = ({ isBuilding }) => {
  ...
  return (
    isBuilding && (
      <div className="bg-gunmetal-600 mt-4 rounded-lg overflow-hidden">
        <button
          type="button"
        ...
        >
          ...
          <span className="text-seasalt-400 text-xs">{isOpen ? '▾' : '▸'}</span>
        </button>
        {isOpen && (
          <div
            ref={scrollRef}
            className="px-3 py-2 max-h-52 overflow-y-auto font-mono text-green-400 text-xs leading-relaxed"
          >
            {logs.length === 0 ? (
